...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        try {
            ...
            client.tempData[`${interaction.user.id}_timeout_roles`] = selectedRoles;
            
            // Step 2: Select Appeal Channel
            const channelSelectMenu = new ActionRowBuilder()
                .addComponents(
                    new ChannelSelectMenuBuilder()
                        .setCustomId('cmd{mod-select-timeout-channel}')
                        .setPlaceholder('Select Appeal Channel')
                        .addChannelTypes(ChannelType.GuildText)
                );
            
            await interaction.update({
                content: '### Step 2: Select Appeal Channel\nPlease select the channel where timeout appeals will be posted.',
                components: [channelSelectMenu]
            });
            
        } catch (error) {
            console.error('Error selecting timeout manager roles:', error);
