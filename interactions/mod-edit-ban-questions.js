...
module.exports = {
    name: 'mod-edit-ban-questions',
    
    run: async (client, interaction, args) => {
        try {
            const config = await ModerationSystem.findOne({ guildId: interaction.guild.id });
            const questions = config.banAppealSystem.questions;
            
            const modal = new ModalBuilder()
                .setCustomId('cmd{mod-update-ban-questions}')
                .setTitle('Edit Ban Appeal Questions');
            ...
            console.error('Error showing ban questions modal:', error);
